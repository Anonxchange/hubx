...
const VideoCard: React.FC<VideoCardProps> = ({ video, viewMode = 'grid' }) => {
  ...
  const handleMouseEnter = () => {
    ...
    // Only use dedicated preview_url to reduce data consumption
    // Don't fallback to main video_url as it wastes bandwidth
    if (video.preview_url && video.preview_url.trim() !== '') {
      setIsHovered(true);
      // Delay preview start by 1000ms to avoid triggering on quick mouse movements
      hoverTimeoutRef.current = setTimeout(() => {
        console.log('Starting preview for:', video.title);
        setShowPreview(true);
        if (videoRef.current && videoRef.current.src !== video.preview_url) {
